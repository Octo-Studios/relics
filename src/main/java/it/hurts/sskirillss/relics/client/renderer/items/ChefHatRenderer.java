...
import it.hurts.sskirillss.relics.client.models.items.ChefHatModel;
...
public class ChefHatRenderer implements IRelicRenderer {
    private final ChefHatModel model;

    public ChefHatRenderer() {
        this.model = new ChefHatModel(Minecraft.getInstance().getEntityModels().bakeLayer(ChefHatModel.LAYER));
    }
    ...
    public <E extends LivingEntity, EM extends EntityModel<E>> void render(ItemStack stack, SlotContext slotContext, PoseStack poseStack, RenderLayerParent<E, EM> parent, MultiBufferSource bufferSource, int light, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.model.renderToBuffer(poseStack, bufferSource.getBuffer(RenderType.entityCutout(FlawlessUtils.getTexture(entity, stack, ResourceLocation.fromNamespaceAndPath(Relics.MODID, "textures/item/model/chef_hat.png")))), relic.getRelicData(entity, stack).isVisuallyFlawless() ? LightTexture.FULL_BRIGHT : light, OverlayTexture.NO_OVERLAY);
