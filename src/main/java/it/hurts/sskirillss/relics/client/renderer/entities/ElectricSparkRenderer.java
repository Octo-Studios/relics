...
@OnlyIn(Dist.CLIENT)
public class ElectricSparkRenderer extends EntityRenderer<ElectricSparkEntity> {
    public ElectricSparkRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(ElectricSparkEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        var end = target.getPosition(partialTicks)
                .add(0D, target.getBbHeight() * 0.5D, 0D)
                .subtract(entity.getPosition(partialTicks));
        var delta = end.subtract(start);
        var length = delta.length();

        if (length < 0.01D || length > entity.getDistance())
            return;
        ...
        var seed = (entity.getId() * 73428767L) ^ (target.getId() * 912931L) ^ now;
    ...
    @Override
    public boolean shouldRender(ElectricSparkEntity entity, @Nonnull Frustum clipping, double x, double y, double z) {
        return true;
    }

    @Override
    public ResourceLocation getTextureLocation(ElectricSparkEntity entity) {
