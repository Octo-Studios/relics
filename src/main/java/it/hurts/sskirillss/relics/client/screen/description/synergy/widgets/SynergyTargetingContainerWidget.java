...
public class SynergyTargetingContainerWidget extends DescriptionContainerWidget {
    ...
    public SynergyTargetingContainerWidget(DescriptionScreen screen) {
        super(screen);
    }

    @Override
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        var data = this.resolveSynergyData();
        ...
        for (var row : rows) {
            if (row.header()) {
                ...
                guiGraphics.drawString(this.minecraft.font, getModeLabel(row.selector()), (this.getX() + TOGGLE_X) * 2, (int) Math.round((y + LABEL_Y_OFFSET) * 2), DescriptionUtils.TEXT_COLOR, false);
            ...
            GUIRenderer.begin(data.getTargetingData().getOption(option, row.selector()) ? SWITCH_ON : SWITCH_OFF, poseStack)
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (var row : rows) {
            ...
            var option = row.option();
            var enabled = !data.getTargetingData().getOption(option, row.selector());

            data.getTargetingData().setOption(option, row.selector(), enabled);
            NetworkHandler.sendToServer(new C2SChangeAbilityTargetingOption(this.getScreen().getContainer(), this.getScreen().getSlot(), data.getId(), true, row.selector().getSerializedName(), option.name().toLowerCase(Locale.ROOT), enabled));
    ...
    private SynergyData resolveSynergyData() {
        var player = this.minecraft.player;
        var stack = this.getScreen().getStack();

        if (player == null || stack == null || !(stack.getItem() instanceof IRelicItem relic) || !(this.getScreen() instanceof SynergyDescriptionScreen screen))
            return null;

        return relic.getRelicData(player, stack).getAbilitiesData().getSynergyData(screen.getSelectedSynergy());
    }

    private List<TargetingRow> getRows(SynergyData data) {
        ...
        return targeting.getSelectors().stream()
                ...
                .flatMap(selector -> {
                    var options = targeting.optionsFor(selector).stream()
                            .filter(targeting::hasOption)
                            .sorted(Comparator.comparingInt(Enum::ordinal))
                            .map(option -> TargetingRow.option(selector, option));

                    return mixed ? java.util.stream.Stream.concat(java.util.stream.Stream.of(TargetingRow.header(selector)), options) : options;
                })
                .toList();
    }
    ...
    private record TargetingRow(SelectorType selector, AbilityTargetingOption option, boolean header) {
