...
void main() {
    ...
    float tearMask = step(0.86, blockNoise) * burst;
    float jitterMask = step(0.985, fineNoise);
    float cellMask = step(0.965, cellNoise) * (0.45 + burst);
    ...
    float waveOffset = sin((uv.y * 90.0) + t * 28.0) * 0.0025 * burst;
    float roll = heavyBurst * sin(time * 0.17) * 0.018;
    ...
    float split = (0.0025 + 0.006 * burst + abs(tearOffset) * 0.28) * (0.65 + hash(block) * 0.7);
    ...
    color += (staticNoise - 0.5) * (0.035 + 0.06 * tearMask);
    ...
    float invertMask = heavyBurst * step(0.992, hash(vec2(block, floor(time * 16.0))));
    color = mix(color, 1.0 - color, invertMask * 0.22);

    float edgeBleed = smoothstep(0.42, 0.74, dist) * (0.2 + 0.35 * burst);
    color.rb += vec2(0.055, 0.085) * edgeBleed;
    color *= 1.0 - smoothstep(0.55, 0.88, dist) * 0.18;
